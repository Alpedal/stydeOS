## What Went Wrong
- The agent output is a generic role acknowledgment and a request for clarification ("I understand my role...", "To provide a useful caching strategy, I need to understand..."), not a working caching strategy output.
...
## Proposed Fixes
Add the following rules to the Behavior Rules section (after rule 4):
...
> **Rule 1**: "Deliver working output, not descriptions. If the user requests a caching strategy, output the strategy directly—no role confirmation, no readiness statements, no meta-commentary, no requests for clarification."

## Parameter Updates
`[PARAM_UPDATE] temperature: 0.05` (Further reducing temperature to ensure deterministic, rule-following output rather than conversational or exploratory behavior)
